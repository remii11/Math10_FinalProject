...
st.title("5-Minute Crafts: Are Views Related to Video Titles?")
...
#intro
st.write("5-Minute Crafts is a popular channel on YouTube, with nearly 75 million subscribers. The channel is often criticized for its misleading aspects such as the thumbnails, which can be considered as 'clickbait'. However, these factors most likely contribute to the large amount of views and video engagement that the channel generates. Although the video thumbnail is a significant factor towards audience engagement, a viewer is likely to be influenced by the title as well. In this project, I will be analyzing the correlation between a variety of aspects of video titles relative to its popularity.")
df = pd.read_csv("5-Minute Crafts.csv")
st.header("Data Cleaning and Confounding Variables")
st.write("Although we are studying the effectiveness of the channel's video titles, the dataset contains other elements that can also affect the popularity as well. Therefore, we will examine the influence of other elements, and minimize this as much as possible in order to keep other categories constant, as not to wrongly influence the results based on the title. ")
